...
## Purpose
HTTP middleware for auth, workspace selection, and request shaping.

## Key Files
| File | Description |
| --- | --- |
| `auth_test.go` | Directory-local file. |
| `auth.go` | Directory-local file. |
| `cloudfront.go` | Directory-local file. |
| `daemon_auth.go` | Directory-local file. |
| `request_logger.go` | Directory-local file. |
| `workspace.go` | Directory-local file. |
