...
## Purpose
Execution-environment helpers used by the daemon when spawning agent work.

## Key Files
| File | Description |
| --- | --- |
| `codex_home.go` | Directory-local file. |
| `context.go` | Directory-local file. |
| `execenv_test.go` | Directory-local file. |
| `execenv.go` | Directory-local file. |
| `git.go` | Directory-local file. |
| `runtime_config.go` | Directory-local file. |
