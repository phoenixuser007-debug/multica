...
type Config struct {
	...
	Agents             map[string]AgentEntry // keyed by provider: claude, codex, copilot, opencode, gemini
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, or gemini and ensure it is on PATH")
	}
