...
type Config struct {
	...
	Agents             map[string]AgentEntry // "claude" -> entry, "codex" -> entry, "copilot" -> entry, "opencode" -> entry
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, or opencode and ensure it is on PATH")
	}
