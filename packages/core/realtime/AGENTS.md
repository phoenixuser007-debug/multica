...
## Purpose
Shared realtime core module that packages reusable domain logic for the monorepo.

## Key Files
| File | Description |
| --- | --- |
| `hooks.ts` | Source file local to this directory. |
| `index.ts` | Barrel export for the directory or package. |
| `provider.tsx` | Provider composition used to wire shared client context. |
| `use-realtime-sync.ts` | Source file local to this directory. |
