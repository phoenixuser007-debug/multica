...
## Purpose
Docs site routes, layouts, and API endpoints.

## Key Files
| File | Description |
| --- | --- |
| `global.css` | Global styling entrypoint for this app surface. |
| `layout.config.tsx` | Route-specific layout configuration for the docs app. |
| `layout.tsx` | Next.js layout component for this route segment. |

## Subdirectories
| Directory | Purpose |
| --- | --- |
| `(home)/` | Docs site route group for the public landing experience. See `(home)/AGENTS.md`. |
| `api/` | Docs site API routes used by search and site utilities. See `api/AGENTS.md`. |
| `docs/` | Top-level docs route entrypoint. See `docs/AGENTS.md`. |
