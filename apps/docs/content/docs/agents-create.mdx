...
The other three (Kimi, OpenCode, Pi), along with each tool's full capability matrix (session resume, MCP, skill injection path, model selection), are covered in [AI coding tools comparison](/providers).
...
```json
["--max-turns", "100", "--append-system-prompt", "keep replies concise"]
```

The final command comes out as:

```bash
claude --model <model> --max-turns 100 --append-system-prompt "keep replies concise" [...]
```
...
- [AI coding tools comparison](/providers) — full capability matrix across all 8 tools
