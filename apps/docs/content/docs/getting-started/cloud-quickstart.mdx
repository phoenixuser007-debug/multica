...
The daemon auto-detects available agent CLIs (`claude`, `codex`, `copilot`, `openclaw`, `opencode`) on your PATH. When an agent is assigned a task, the daemon creates an isolated environment, runs the agent, and reports results back.
