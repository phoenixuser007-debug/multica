...
This is **exactly** the filter the IoTOps team uses in the Humio UI — do not change it without team sign-off:

```lsql
"kubernetes.container_name" = "iotops-client*"
AND !"kubernetes.container_name" = "*location*"
...
- `"*location*"` exclusion removes `iotops-client-location-*` repos — those are handled by a different monitor.
