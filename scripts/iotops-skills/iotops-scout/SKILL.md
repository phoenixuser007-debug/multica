---
name: iotops-scout
description: "Daily scan of 4 Humio clusters (aqua, brooke, jedi, firth) for iotops-client-* runtime errors. For each distinct error: fetch ±10-min pod context, look up the affected source file(s) in Stash, classify by fingerprint, create a JIRA CNX bug (or reuse an existing one), and hand off to the IoTOps Fixer agent as a child multica issue. Triggers on issues whose title starts with 'Scan iotops-client-*' — created by the IoTOps Error Scout autopilot."
allowed-tools: ["Read", "Bash", "Grep", "Glob"]
---

# IoTOps Scout

You are the **first link** in the 3-agent IoTOps error chain. An autopilot fires you once a day (schedule set in the multica UI). Your job: scan 4 clusters, classify each distinct error by looking at the actual code, create Jira + Fixer child issue, hand off.

## When this skill fires

Your issue title starts with `Scan iotops-client-*`. The issue was created by the **IoTOps Error Scout autopilot** on its scheduled trigger.
...
- `MULTICA_API_URL`, `MULTICA_TOKEN`, `MULTICA_WORKSPACE_ID`, `IOTOPS_FIXER_AGENT_ID` — to create the child issue
...
The autopilot runs daily. Use a **24-hour sliding window ending at `now`** (or whatever the time since the previous Scout tick — read `created_at` of the previous tick issue via `GET /api/issues?q=Scan+iotops-client-*&limit=2&order=-created_at` and use that as `start`, else fall back to 24 h).
...
```json
{"pod": "...", "service_repo": "iotops-client-state-processor",
